...
public class Knight extends ChessPiece {
    public Knight(Settings settings, Role role) {
        super(settings, role);
    }

    @Override
    public ItemStack getWhiteStack() {
        return ModItems.WHITE_KNIGHT.getDefaultStack();
    }

    @Override
    public ItemStack getBlackStack() {
        return ModItems.BLACK_KNIGHT.getDefaultStack();
    }
    ...
    protected Map<PieceAction, List<Vector2i>> calculateStandardMoves(int x, int y, int role, ChessGrid grid, List<Vector2i> moveCells, List<Vector2i> takeCells, boolean allPossible) {
        List<Vector2i> tempCells = List.of(new Vector2i(x + 1, y - 2), new Vector2i(x - 1, y - 2), new Vector2i(x - 2, y - 1),
                new Vector2i(x - 2, y + 1), new Vector2i(x - 1, y + 2), new Vector2i(x + 1, y + 2), new Vector2i(x + 2, y - 1), new Vector2i(x + 2, y + 1));

        for (Vector2i pos : tempCells){
            int pieceId = grid.safeGetPieceId(pos.x, pos.y);
            if (pieceId != -1){
                if (allPossible && ChessGrid.getPieceRole(pieceId) == role) moveCells.add(pos);
                if (pieceId == ChessPieces.EMPTY.id)
                    moveCells.add(pos);
                else if(ChessGrid.getPieceRole(pieceId) != role)
                    takeCells.add(pos);
            }
        }
